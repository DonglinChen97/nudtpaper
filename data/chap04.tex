...
\texttt{GCN}、\texttt{GCN+CFD}和\texttt{P\_CFD}

本节将基于GCN模型的模拟结果称为\texttt{GCN}模拟结果；
将基于GCN模型的模拟结果并进一步利用CFD求解器进行优化的结果称为\texttt{GCN+CFD}模拟结果；
将传统的CFD求解器计算结果称为\texttt{P\_CFD}模拟结果。


表\ref{tab:gcn_comp}展示了\texttt{GCN}、\texttt{GCN+CFD}和\texttt{P\_CFD}三种方法在测试集上的性能表现。
其中\texttt{GCN+CFD}和\texttt{P\_CFD}的预测结果
