...
if __name__ == '__main__':
    ...
    index,loss_train1,loss_test1 = df("result_test_new_baseline_myloss_.txt")

    _,loss_train2,loss_test2 = df("result_test_new_relu_myloss_400_.txt")

    _,loss_train3,loss_test3 = df("result_test_new_relu_CBAM_half_myloss_400_.txt")
    ...
    #plt.plot(index, loss_test1, '-',linewidth=2.0,
    #         color='orangered', alpha=0.8, label='Theuerey et al\'s')
    
    loss_test2 = [x * 1000 for  x in loss_test2]
    plt.plot(index, loss_test2, '-',linewidth=2.0,
             c='turquoise', alpha=0.8, label='FlowDNN')
    
    loss_test3 = [x * 1000 for x in loss_test3 ]
    plt.plot(index, loss_test3, '-',linewidth=2.0,
             color='#4169E2', alpha=0.8, label='FlowDNN w/ AM')
