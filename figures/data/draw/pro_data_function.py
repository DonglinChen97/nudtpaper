...
if __name__ == '__main__':
    ...
    index,loss_train1,loss_test1 = df("result_test_new_elu_myloss.txt")

    _,loss_train2,loss_test2 = df("result_test_new_elu_nobn_myloss.txt")

    _,loss_train3,loss_test3 = df("result_test_new_relu_myloss.txt")
...
#orangered
    ...
    plt.plot(index, loss_test1, '-',linewidth=2.0,
             color='#4169E2', alpha=0.8, label='ELU w/ BN')
    
    plt.plot(index, loss_test2, '-',linewidth=1.8,
             c='plum', alpha=0.8, label='ELU w/o BN')
    
    plt.plot(index, loss_test3, '-',linewidth=2.0,
             color='orangered', alpha=0.8, label='ReLU w/ BN')
    #plt.plot(index, loss_test2, '-',linewidth=1.8,
    #         color='turquoise', alpha=0.8, label='Ours for validation')
